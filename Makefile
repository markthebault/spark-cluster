...
build-jupyter:
	...
	sed -i "s/%%SPARK_VERSION%%/$(SPARK_VERSION)/g" .tmp/Dockerfile
	sed -i "s/%%HADOOP_VERSION%%/$(HADOOP_VERSION)/g" .tmp/Dockerfile
	sed -i "s#%%TOREE_PACKAGE%%#$(TOREE_PACKAGE_SPARK_16)#g" .tmp/Dockerfile
	cd .tmp && docker build -t jupyter-spark:${SPARK_VERSION} .
	rm -Rf .tmp

create-docker-compose:
	cp docker-compose.yml.template docker-compose-builded.yml
	sed -i "s/__SPARK_IMAGE_NAME__/spark:$(SPARK_VERSION)/g" docker-compose-builded.yml
	sed -i "s/__JUPYTER_IMAGE_NAME__/jupyter-spark:$(SPARK_VERSION)/g" docker-compose-builded.yml 
